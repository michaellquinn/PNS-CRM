...
/* The "paste an opportunity ID" card lived here until 2026-09-11 (Michael). It queued
   the id for the sync and the ticket appeared a few minutes later — which is exactly
   what the Import queue screen does, for exactly the same people: both were gated on
   queueSync, so nothing and nobody lost a capability by dropping this one. Two ways to
   do one thing, on two screens, is how they drift.

   What remains on this screen is the manual form, which is for a deal that is NOT in
   Sales CRM yet: it parks the ticket in Pending CRM ID until an opportunity id arrives.
   That distinction used to be explained by the card being deleted; it is now the first
   line of the form's own description. */
/* ---------------------------------------------------------------- new request */
export function NewRequest({ me, notify, onCreated }) {
  ...
  // Must Win is not offered here: it is set in Sales CRM only and arrives with the sync.
  const watched = ["Hypercare", "Strategic"].includes(f.acct_type) ? f.acct_type : null;
  ...
  return (
    <>
      <Head title="New request"
        sub="For a deal that is NOT in Sales CRM yet — it parks in Pending CRM ID until an opportunity id is added. Already in Sales CRM? Use Import queue instead; the sync builds the ticket from the record Sales already filled in. One service per request, and the Project Charter is generated once PNS clears the input."
