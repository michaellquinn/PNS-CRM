...
const ENTRIES = [
  ...
  },
  {
    ...
    changes: [
      ...
      "FIXED before it shipped: a blank tier in Sales CRM would have demoted accounts. The tier is read from Account.customer_success_manager, a free-text field that also holds legacy Salesforce ids and junk, and which has no way to say “Standard” — so “no tier found” and “demoted to Standard” look identical. With the 5-minute sync that would have reset a Hypercare account to Standard every five minutes, silently, and the tier decides who prices the deal, PSP entry, the watched groups and C-level sign-off. Sales CRM can now PROMOTE an account but not silently demote one; a real demotion is done by hand here.",
