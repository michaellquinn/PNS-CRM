...
const ENTRIES = [
  {
    ...
    changes: [
      ...
      "Readiness cards are ONE PER TEAM, and every point is mapped field by field from the form (Michael, 2026-09-25). CL: packing tags (one point whatever the tags) and insurance/claim. DE: load, packing, COD, RDO and order creation when it is theirs. Sort: the sort handling note, TKBM and implan. The pickup team: load, address and PIC, vehicle, window and waiting time, driver, TKBM, implan, RDO, Surat Jalan and the pickup handling note. The delivery team: destination, vehicle, window and waiting time, driver, TKBM, POD, RDO, Surat Jalan and the delivery handling note. Sections A, the ids, the service, the mode and the two responsibility pickers raise nothing — they say what the launch is.",
      "A point exists only when there is something to confirm: COD No, RDO No, an empty Surat Jalan or handling note, an order creation somebody else does, and TKBM or implan set to No all raise nothing.",
      "Where two teams must both agree — the load, RDO, TKBM, packing — each gets its own point and confirms separately.",
  ...
  },
  {
    ...
    changes: [
      ...
      "B2C is named PSP Calculator too, and opens the calculator as well. A name with no link renders as plain text, so the name alone would have named a tool and then refused to open it — worse than the document name it replaced. B2C prices off the B2BR card and that is the same tool.",
    ],
    overruled: [],
  },
  {
  ...
  },
  {
    ...
    changes: [
      ...
      "Also fixed, found by the new reporting: the refresh rotation skipped rotating whenever the held-ticket pool fit under its ceiling, and advanced its cursor by ids fetched rather than tickets refreshed. Both left a stable tail of the pool never refreshed at all — the app holds 37 held tickets and a run got through 23, so the same 14 were skipped every time. That is the exact bug the cursor was written to fix in August, reached from a new angle. It rotates unconditionally now and moves by work actually done.",
      "A dry run no longer moves the refresh rotation. It writes nothing to the database either way, but the rotation cursor was the one piece of state it could still shift — and shifting it pushes real tickets a whole cycle down the queue because somebody pressed a button that promises to change nothing.",
      "Third fan-out closed: warming the accounts behind newly discovered opportunities. A manual 7-day run measured 25.4 seconds with nothing scanned at all — the entire budget spent looking up accounts and no time left to import one of them. Accounts for opportunities somebody asked for by id stay unbounded, because an import without its account fails outright; accounts for deals the sweep found on its own are bounded, because the next run finds them again anyway.",
